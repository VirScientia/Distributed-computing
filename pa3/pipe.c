...
bool runChildProcess(FILE *logFile, struct Ipc ipc) {
    ...
    printLog(logFile,
             log_started_fmt,
             get_lamport_time(&ipc), ipc.id, ipc.pid, ipc.parentPid, ipc.balance);
    ...
    local_id processAmount = ipc.n;
    receiveAnyBlocking(&ipc, processAmount);
    ...
    while (exp) {
        ...
        const timestamp_t endTime = get_lamport_time(&ipc);
        ...
        switch (historyMessage.s_header.s_type) {
            case STOP: {
                ...
                printLog(logFile,
                         log_done_fmt,
                         get_lamport_time(&ipc), ipc.id, ipc.balance);
            ...
            case TRANSFER: {
                ...
                if (ipc.id == order->s_dst) {
                    ...
                    for (timestamp_t timeL = historyMessage.s_header.s_local_time - 1;
                         timeL < endTime; timeL++) {
                        history.s_history[timeL].s_balance_pending_in += order->s_amount;
                        //printf("timeL = %d, id = %d\n", timeL, ipc.id);
                    }
    ...
    history.s_history[len].s_balance_pending_in = 0;
